...
func (da *TableTypeActivity) T(queryable mingru.Queryable, id uint64, limit int, offset int, max int) ([]*ActivityTableTResult, int, error) {
	rows, err := queryable.Query("(SELECT `id`, `sig` AS `generic_sig`, `url_name` AS `generic_name` FROM `user` WHERE `id` = ? LIMIT ? OFFSET ?) UNION ALL (SELECT `user_id`, `value` FROM `like`) UNION (SELECT `title` FROM `db_post` WHERE `id` = ?) LIMIT ? OFFSET ?)", id, limit, offset, id, limit, offset)
	if err != nil {
		return nil, 0, err
	}
